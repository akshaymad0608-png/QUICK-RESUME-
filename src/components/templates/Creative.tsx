  ...
  return (
    <div className="flex flex-col w-full h-full bg-white text-gray-800" style={{ fontSize: design.fontSize, lineHeight: design.lineHeight }}>
      ...
      <div className="relative overflow-hidden" style={{ minHeight: '300px' }}>
        ...
        <div className="relative z-10 px-12 pt-16 pb-12 text-slate-900 flex justify-between items-start gap-8">
          ...
          {personalInfo.photoUrl && data.design.showPhoto !== false && (
             <img src={personalInfo.photoUrl} alt="Profile" className="w-32 h-32 rounded-2xl object-cover shadow-2xl border-4 shrink-0" style={{ borderColor: 'white' }} />
          )}
        </div>
      </div>
      ...
      <div className="flex px-12 -mt-12 relative z-20 gap-12 flex-1 pb-12">
        <div className="w-[38%] space-y-10">
           {/* Contact Card */}
           <div className="bg-gray-900 text-slate-100 p-8 rounded-2xl shadow-2xl">
           ...
           {certifications.length > 0 && (
             <div className="p-4">
               <h3 className="text-lg font-black uppercase mb-5" style={{ fontFamily: hf, color: c }}>Certifications</h3>
               <div className="flex flex-col gap-4">
                 {certifications.map((cert) => (
                   <div key={cert.id}>
                     <div className="text-[14px] font-bold text-gray-800" style={{ fontFamily: hf }}>{cert.name}</div>
                     {cert.issuer && <div className="text-[13px] text-gray-600 mt-1">{cert.issuer}</div>}
                     <div className="text-[12px] font-bold text-gray-400 mt-1 uppercase tracking-wider">{cert.date}</div>
                   </div>
                 ))}
               </div>
             </div>
           ...
           {languages.length > 0 && (
             <div className="p-4">
               <h3 className="text-lg font-black uppercase mb-5" style={{ fontFamily: hf, color: c }}>Languages</h3>
               <div className="flex flex-col gap-3">
                 {languages.map((lang) => (
                   <div key={lang.id} className="flex justify-between items-center border-b border-gray-100 pb-2 last:border-0">
                     <span className="text-[14px] font-bold text-gray-800" style={{ fontFamily: hf }}>{lang.name}</span>
                     <span className="text-[12px] font-medium text-gray-500 uppercase tracking-wider">{lang.proficiency}</span>
                   </div>
                 ))}
               </div>
             </div>
        ...
        <div className="w-[62%] pt-16 space-y-12">
          {summary && (
            <section>
              ...
              <p className="text-[15px] leading-[1.8] text-gray-600 font-medium whitespace-pre-wrap" style={{ fontFamily: bf }}>{summary}</p>
            </section>
          )}
          {experience.length > 0 && (
             <section>
               ...
               <div className="space-y-8">
                 {experience.map((exp) => (
                   <div key={exp.id}>
                     ...
                     <p className="text-[14.5px] leading-relaxed text-gray-600 whitespace-pre-wrap" style={{ fontFamily: bf }}>
                       {exp.description}
                     </p>
                   </div>
                 ))}
          ...
          {projects.length > 0 && (
             <section>
               <h3 className="text-xl font-black uppercase mb-6 tracking-wide" style={{ fontFamily: hf, color: c }}>Projects</h3>
               <div className="space-y-8">
                 {projects.map((proj) => (
                   <div key={proj.id}>
                     <div className="flex justify-between items-end mb-1">
                       <h4 className="font-bold text-[18px] text-gray-900 leading-none" style={{ fontFamily: hf }}>
                         {proj.title}
                     ...
                     {proj.subtitle && (
                       <div className="text-[14px] font-bold uppercase tracking-widest mb-3" style={{ color: c }}>{proj.subtitle}</div>
                     )}
                     {proj.description && (
                       <p className="text-[14.5px] leading-relaxed text-gray-600 whitespace-pre-wrap" style={{ fontFamily: bf }}>
                         {proj.description}
               ...
               <div className="space-y-6">
                 {education.map(edu => (
                   <div key={edu.id}>
                      ...
                      {edu.description && (
                        <p className="text-[14.5px] leading-relaxed text-gray-600 mt-2 whitespace-pre-wrap" style={{ fontFamily: bf }}>{edu.description}</p>
                      )}
                   </div>
                 ))}
               </div>
