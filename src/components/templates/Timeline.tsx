      ...
      <header className="p-10 bg-white shadow-sm flex justify-between items-center z-10 border-t-8" style={{ borderColor: c }}>
        <div className="flex items-center gap-6">
          {personalInfo.photoUrl && data.design.showPhoto !== false && (
             <img src={personalInfo.photoUrl} alt="Profile" className="w-20 h-20 rounded-full object-cover shadow-sm border border-gray-200 shrink-0" />
          )}
          <div>
      ...
      <div className="flex flex-1 p-10 gap-10">
        <div className="w-[65%] space-y-10">
                 ...
                 {experience.map((exp) => (
                   <div key={exp.id} className="relative pl-6 pb-8 last:pb-0">
                     ...
                     <div className="bg-white p-6 md:p-7 rounded-xl shadow-sm border border-gray-100 mt-[-6px]">
                       ...
                       <p className="text-[14px] leading-relaxed text-gray-600 border-t border-gray-50 pt-3 whitespace-pre-wrap" style={{ fontFamily: bf }}>
                         {exp.description}
                       </p>
                     </div>
                   </div>
           ...
           {projects.length > 0 && (
             <section>
               <h3 className="text-lg font-black uppercase mb-6 tracking-wider flex items-center gap-3" style={{ fontFamily: hf, color: c }}>
                 <span className="w-8 h-px bg-current"></span> Projects
               </h3>
               <div className="space-y-0 pl-4 border-l-4 ml-2" style={{ borderColor: c }}>
                 {projects.map((proj) => (
                   <div key={proj.id} className="relative pl-6 pb-8 last:pb-0">
                     <div className="absolute left-[-11px] top-0 w-4 h-4 rounded-full border-[3px] border-[#f8fafc]" style={{ backgroundColor: c }}></div>
                     <div className="bg-white p-6 md:p-7 rounded-xl shadow-sm border border-gray-100 mt-[-6px]">
                       <div className="flex justify-between items-center mb-2">
                       ...
                       {proj.subtitle && (
                         <div className="text-[14px] font-bold mb-3" style={{ color: c }}>{proj.subtitle}</div>
                       )}
                       {proj.description && (
                         <p className="text-[14px] leading-relaxed text-gray-600 border-t border-gray-50 pt-3 whitespace-pre-wrap" style={{ fontFamily: bf }}>
                           {proj.description}
        ...
        <div className="w-[35%] space-y-10">
               ...
               <div className="space-y-4 pl-4 border-l-4 ml-2 border-gray-200">
                  {education.map(edu => (
                    <div key={edu.id} className="relative pl-6 pb-4">
                      ...
                      {edu.description && (
                        <p className="text-[13px] text-gray-600 mt-2 whitespace-pre-wrap">{edu.description}</p>
                      )}
                    </div>
                  ))}
               </div>
           ...
           {certifications.length > 0 && (
             <section>
               <h3 className="text-lg font-black uppercase mb-6 tracking-wider flex items-center gap-3" style={{ fontFamily: hf, color: c }}>
                 <span className="w-6 h-px bg-current"></span> Certifications
               </h3>
               <div className="space-y-4 pl-4 border-l-4 ml-2 border-gray-200">
                  {certifications.map(cert => (
                    <div key={cert.id} className="relative pl-6 pb-4">
                      <div className="absolute left-[-9px] top-0 w-3 h-3 rounded-full bg-gray-300"></div>
                      <div className="font-bold text-[14px] text-gray-900" style={{ fontFamily: hf }}>{cert.name}</div>
                      {cert.issuer && <div className="text-[13px] text-gray-700 my-1 font-medium">{cert.issuer}</div>}
                      <div className="text-[12px] font-bold text-gray-500">{cert.date}</div>
                    </div>
                  ))}
               </div>
             </section>
           ...
           {languages.length > 0 && (
             <section>
               <h3 className="text-lg font-black uppercase mb-6 tracking-wider flex items-center gap-3" style={{ fontFamily: hf, color: c }}>
                 <span className="w-6 h-px bg-current"></span> Languages
               </h3>
               <div className="flex flex-col gap-3 pl-4 border-l-4 ml-2 border-gray-200">
                  {languages.map(lang => (
                    <div key={lang.id} className="relative pl-6 pb-2">
                      <div className="absolute left-[-9px] top-1.5 w-3 h-3 rounded-full bg-gray-300"></div>
                      <div className="font-bold text-[14px] text-gray-900" style={{ fontFamily: hf }}>{lang.name}</div>
                      <div className="text-[13px] text-gray-700 font-medium">{lang.proficiency}</div>
                    </div>
                  ))}
               </div>
             </section>
