...
export const ChatAssistant: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
      <div className="bg-gradient-to-r from-slate-800 to-black p-4 text-slate-900 shrink-0 flex items-center gap-3">
        <Bot size={24} />
        <div>
          <h2 className="font-bold text-lg leading-tight">AI Assistant</h2>
          <p className="text-slate-400 text-xs">Interactive resume guidance</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-3 \${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`px-4 py-3 rounded-2xl max-w-[85%] \${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
      ...
      <div className="p-3 bg-gray-50 border-t border-gray-100 shrink-0">
        <div className="relative">
          <textarea
            ...
            className="w-full resize-none rounded-xl border border-gray-200 py-3 pl-4 pr-12 focus:outline-none focus:ring-2 focus:ring-slate-400 bg-white shadow-sm"
            rows={2}
          />
          <button 
            onClick={handleSend}
